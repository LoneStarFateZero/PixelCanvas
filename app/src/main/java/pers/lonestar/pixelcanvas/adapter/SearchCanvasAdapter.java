...
public class SearchCanvasAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    // 正在加载
    ...
    public SearchCanvasAdapter(List<BmobCanvas> bmobCanvasList) {
        super();
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == TYPE_ITEM) {
            ...
            return new SearchCanvasAdapter.RecyclerViewHolder(view);
        }
        //添加FootView
        else if (viewType == TYPE_FOOTER) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.footview, parent, false);
            return new SearchCanvasAdapter.FootViewHolder(view);
        }
        return null;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof SearchCanvasAdapter.RecyclerViewHolder) {
            final SearchCanvasAdapter.RecyclerViewHolder recyclerViewHolder = (SearchCanvasAdapter.RecyclerViewHolder) holder;
            ...
            //缩略图
            Glide.with(SearchActivity.getInstance()).load(ParameterUtils.bytesToBitmap(bmobCanvas.getThumbnail())).into(recyclerViewHolder.thumbnail);
            //设置当前头像
            ...
            recyclerViewHolder.thumbnail.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(SearchActivity.getInstance(), CanvasInfoActivity.class);
                    intent.putExtra("pixel_canvas", bmobCanvas);
                    intent.putExtra("pixel_user", bmobCanvas.getCreator());
                    SearchActivity.getInstance().startActivity(intent);
                }
            });
        } else if (holder instanceof SearchCanvasAdapter.FootViewHolder) {
            SearchCanvasAdapter.FootViewHolder footViewHolder = (SearchCanvasAdapter.FootViewHolder) holder;
            switch (loadState) {
                case LOADING: // 正在加载
